<template>
    <div>
        ...
        text-sm text-white shadow horver:border-indigo-600 horver:bg-indigo-600
        hover:text-white focus:border-indigo:600 focus:bg-indigo:600
        focus:text-white focus:shadow-none"> Create
            User
        </button>
        </form>
        <!-- <p v-if="notification" :class="notification.type">{{ notification.messagge }}</p> -->
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState("notfication", ["notification"]),
        },
        methods: {
